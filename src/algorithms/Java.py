        ...
        if(re.search("<name><name>Map</name>", a) == None):
            ...
            for tipoId in stringFor:
                ...
                if(re.search('</type>|<type>', tipoId)):
                    ...
                    if(re.search('\w+</name><index>.*</type>', a2) == None):
                        ...
                        if(len(tipo) > 0):
                            ...
                            if(re.search('<name>\w+</name><argument_list', tipoId)):

                                if(re.search('&lt;<argument><name>\w+</name></argument>&gt;', tipoId)):

                                    listTypes = re.findall(
                                        '&lt;<argument><name>\w+</name></argument>&gt;', tipoId)

                                    if(len(listTypes) > 0):

                                        listType = re.findall(
                                            '<name>\w+</name>', listTypes[0])

                                        if(len(listType) > 0):
                                            tipoCertoDaLista = sub(listType[0])
                                            tipo = tipoCertoDaLista
                ...
                if(codId == True and codTipo == True):
                    ...
                    addLine = [identificador, tipo, posicao,
                               arq, classe, metodo]

                    with open('Identifiers.csv', 'a') as file:
...
def run(arq):
    ...
    for i in range(tamanhoString):
        ...
        # identificadoresAtributo = subprocess.Popen(
        #     'srcml --xpath "//src:decl[not(ancestor::src:for) and not(ancestor::src:while) and not(ancestor::src:do) and not(ancestor::src:if_stmt) and not(ancestor::src:switch) and not(ancestor::src:case) and not(ancestor::src:function) and not(ancestor::src:parameter_list) and not(ancestor::src:class)] " classeXpath.xml', shell=True, stdin=subprocess.PIPE, stdout=subprocess.PIPE, stderr=subprocess.PIPE)

        identificadoresAtributo = identificadoresAtributo.stdout.read()
        identificadoresAtributo = identificadoresAtributo.decode(
            'utf-8').split("</unit>")

        # print(identificadoresAtributo)

        # TIRAR O COMENTAROO DEPOIS AQUII

        catchTypeId(identificadoresAtributo, "Atributo",
                    arq, nomeClasseUse, None)
